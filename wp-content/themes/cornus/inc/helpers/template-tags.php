...
function Cornus_the_excerpt($trim_character_count = 0)
{
    ...
    $excerpt = wp_strip_all_tags(get_the_excerpt()); //remove all the unnecessary html tags
    $excerpt = substr($excerpt, 0, $trim_character_count); // retrieves a piece of string

    $excerpt = substr($excerpt, 0, strrpos($excerpt, " ")); // remove the last word 

    echo $excerpt . "[...]";
}

//function to add read more in excerpt
function Cornus_read_more($more = '')
{
