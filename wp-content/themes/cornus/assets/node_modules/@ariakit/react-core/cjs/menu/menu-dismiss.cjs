"use strict";Object.defineProperty(exports, "__esModule", {value: true});"use client";


var _NY3D6JQTcjs = require('../__chunks/NY3D6JQT.cjs');


var _NGEKJJDHcjs = require('../__chunks/NGEKJJDH.cjs');
require('../__chunks/7LBZXSLY.cjs');
require('../__chunks/UXFBTIBY.cjs');
require('../__chunks/7ZVTEFD7.cjs');
require('../__chunks/5UTRYT6E.cjs');
require('../__chunks/DAJUUBUI.cjs');
require('../__chunks/UZNYSPKP.cjs');
require('../__chunks/BZTDJIVT.cjs');
require('../__chunks/3W6FH6JK.cjs');
require('../__chunks/UVBBMANL.cjs');
require('../__chunks/F2A2ZQDB.cjs');
require('../__chunks/S6UU7NA4.cjs');
require('../__chunks/Z3GCTNW4.cjs');
require('../__chunks/75KXQZJX.cjs');




var _RNZNGEL4cjs = require('../__chunks/RNZNGEL4.cjs');
require('../__chunks/EO6LS72H.cjs');
require('../__chunks/CJDHQUBR.cjs');



var _AV6KTKLEcjs = require('../__chunks/AV6KTKLE.cjs');

// src/menu/menu-dismiss.ts
var useMenuDismiss = _RNZNGEL4cjs.createHook.call(void 0, 
  (_a) => {
    var _b = _a, { store } = _b, props = _AV6KTKLEcjs.__objRest.call(void 0, _b, ["store"]);
    const context = _NGEKJJDHcjs.useMenuScopedContext.call(void 0, );
    store = store || context;
    props = _NY3D6JQTcjs.useHovercardDismiss.call(void 0, _AV6KTKLEcjs.__spreadValues.call(void 0, { store }, props));
    return props;
  }
);
var MenuDismiss = _RNZNGEL4cjs.createComponent.call(void 0, (props) => {
  const htmlProps = useMenuDismiss(props);
  return _RNZNGEL4cjs.createElement.call(void 0, "button", htmlProps);
});
if (process.env.NODE_ENV !== "production") {
  MenuDismiss.displayName = "MenuDismiss";
}



exports.MenuDismiss = MenuDismiss; exports.useMenuDismiss = useMenuDismiss;
